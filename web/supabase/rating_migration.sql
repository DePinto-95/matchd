...
-- NOTE: hardened version (upward-correction cap + rater credibility weight).
-- Kept in sync with rating_hardening.sql — edit both together.
...
BEGIN
  ...
  -- EMA peer update with the credibility-scaled learning rate
...
-- ============================================================
-- 4. CREATE update_elo_rating TRIGGER (fires on confirmed/abandoned result)
-- ============================================================
...
DECLARE
  ...
  v_combined     NUMERIC;
BEGIN
  -- Must be an UPDATE (never fires on INSERT)
  ...
  -- ABANDONED: match happened but result was unresolved.
  -- Increment total_matches (counts toward the display gate) but do NOT apply Elo.
